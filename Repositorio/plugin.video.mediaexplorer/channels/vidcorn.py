...
def login():
    ...
    logged_in = scrapertools.find_single_match(data,'data-user-logged-in="([^"]+)') == 'true'
...
def seasons(item):
    ...
    for season, url, body in scrapertools.find_multiple_matches(data, patron):
        patron = '<span class="viewed pull-right"><small data-toggle="tooltip" title="(\d+)'
        for enlaces in scrapertools.find_multiple_matches(body, patron):
            if int(enlaces) > 0:
...
def episodes(item):
    ...
    patron = '''<div data-episodio="([^"]+)".*?'''
    patron += '''<span class='num_ep'>(\d+).*?<span class='nom_ep'>([^<]+).*?<small data-toggle="tooltip" title="(\d+)'''

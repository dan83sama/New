...
def episodes(item):
    ...
    for episode in episode_list:
        itemlist.append(item.clone(
            action='findvideos',
            label=episode['title'].get('es', episode['title']['en']).strip('" '),
            ...
            title=episode['title'].get('es', episode['title']['en']).strip('" '),
            hdfulltype='episodes',
            hdfullid=episode['id'],
...
def findvideos(item):
    ...
    providers = httptools.downloadpage(host + "/js/providers.js?v=3.0.53").data
    providers = jhexdecode(providers)
    ...
    providers = dict([(
        ...
        {
            'stream': p[1] == 's',
            'url': eval(re.sub(r'var_0x1' , '"%s"', p[2]))
        }
    ) for p in providers])
    ...
    for server in servers:
        itemlist.append(item.clone(
            action='play',
            url=providers.get(server['provider'])['url'] % server['code'],
            type='server',
            lang=LNG.get(server['lang'].lower()),
            quality=QLT.get(server['quality']),
            stream=providers.get(server['provider'])['stream']
        ))
...
def jhexdecode(t):
    vars = set(scrapertools.find_multiple_matches(t, '(_0x[a-f0-9]+)'))
    for i, v in enumerate(vars):
        t = t.replace('%s' % v, 'var_%s' % i)
    ...
    t = re.sub(r'\\x(\w{2})', to_hx, t).replace('var ', '')
    f = eval(re.findall('\s*var_1\s*=\s*([^;]+);', t, flags=re.DOTALL)[0])

    for i, v in enumerate(f):
        if v == "":
            t = t.replace('var_1[%s]' % i, '""')
        else:
            t = t.replace('[var_1[%s]]' % i, "." + f[i])
            t = t.replace('var_1[%s]' % i, '"' + f[i] + '"')

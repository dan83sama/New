...
LNG = Languages({
    ...
    Languages.vos: ['VS', 'vose', 'VOSE'],
    Languages.en: ['en']
})

QLT = Qualities({
    Qualities.scr: ['TS', 'TC-HQ', 'ts-hq', 'TS-HQ', 'br-s', 'dvd-s'],
    Qualities.hd: ['HD-R', 'hd-r', 'HD-TV', 'HDTV', 'Micro-HD-720p'],
    Qualities.hd_full: ['BR-R', 'Micro-HD-1080p'],
    Qualities.rip: ['DVD-R', 'dvd-r']
})
...
def movie_search(item):
    ...
    data = httptools.downloadpage('https://cse.google.es/cse?cx=014793692610101313036:vwtjajbclpq&q=').data
    cse_token = scrapertools.find_single_match(data, "var cse_token='([^']+)'")

    if not item.url:
        item.url = 'https://www.googleapis.com/customsearch/v1element?' \
    ...
    data = jsontools.load_json(httptools.downloadpage(item.url, no_decode=True).data)
...
def newest(item):
    ...
    data = httptools.downloadpage(item.url or M_HOST).data
    data = re.sub(r"\n|\r|\t|\s{2}|&nbsp;", "", data)
    ...
    for url, title, year, lng, qlt, poster in scrapertools.find_multiple_matches(data, patron):
        ...
        itemlist.append(item.clone(
            ...
            year=year,
            quality=QLT.get(qlt),
            lang=[LNG.get(l.strip().split(' ')[0]) for l in lng.split(',')]
        ))
...
def movies(item):
    ...
    for url, title, poster, year, langs, qlt in results[item.index:item.index + 40]:
        itemlist.append(item.clone(
            ...
            action='findvideos',
            quality=QLT.get(qlt),
            lang=[LNG.get(l) for l in scrapertools.find_multiple_matches(langs, '\(([^)]+)\)')],
            year=year
        ))
...
def episodes(item):
    ...
    for url, season, episode, title, langs in scrapertools.find_multiple_matches(data, patron):
        ...
        itemlist.append(item.clone(
            ...
            type='episode',
            content_type='servers',
            lang=[LNG.get(l) for l in scrapertools.find_multiple_matches(langs, '/img/lng/([^.]+)\.png')]
        ))
...
def newest_episodes(item):
    ...
    data = httptools.downloadpage(item.url or T_HOST + '/inc/index.php?bloque=1&idioma=all&page=1').data
    data = re.sub(r"\n|\r|\t|\s{2}|&nbsp;", "", data)
...
def play(item):
    ...
    result = Anticaptcha().solve_recaptcha(url=T_HOST, site_key=site_key)

    post = {
        'g-recaptcha-response': result,
        'submit': 'Reproducir'
    }
    item.url = httptools.downloadpage(item.url, post=post, follow_redirects=False, only_headers=True).headers[
        'location']

...
class Client(object):
    ...
    def prioritize_file(self):
        ...
        for i in xrange(self.meta.num_pieces()):
            if self.file.first_piece <= i <= self.file.last_piece:
                priorities.append(1)
            else:
                priorities.append(0)
    ...
    def status(self):
        ...
        if self._th:
            ...
            # Tamaño del archivo
            if self.file:
                st['file_size'] = self.file.size / 1048576.0
            else:
                st['file_size'] = 0
            ...
            if self.file and self.file.cursor:  # Con una conexion activa: Disponible vs Posicion del reproductor
                ...
                # 2. Buffer al final del archivo (en algunos archivos el reproductor mira el final del archivo antes
                # de comenzar)
                bp = []
                ...
                st['buffer']= int(sum(bp) * 100 / self.buffer_size)
            ...
            if self._ses.dht_state() is not None:
                st['dht_state'] = "On"
                st['dht_nodes'] = self._ses.status().dht_nodes
            else:
                st['dht_state'] = "Off"
                st['dht_nodes'] = 0

            # Cantidad de Trackers
            st['trackers'] = len(self._th.trackers())
    ...
    def priorize_start_file(self):
        ...
        if self.file and not self.file.cursor:
            ...
            for y in range(self.file.last_piece - self.last_pieces_priorize, self.file.last_piece + 1):
    ...
    def print_status(self):
        ...
        if self.file:
            ...
            archivo = "N/D"
        logger.info('%.2f%% de %.1fMB %s | %.1f kB/s | #%s %d%% | AutoClose: %s | S: %d(%d) P: %d(%d)) | '

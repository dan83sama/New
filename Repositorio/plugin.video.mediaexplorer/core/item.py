...
class Item(object):
    ...
    def __setattr__(self, key, value):
        ...
        if key == 'type' and not value in self.valid_types:
            raise Exception('El tipo de ítem no es válido: \'%s\'' % value)

        if key == 'content_type' and not value in self.valid_contents:
    ...
    def clone(self, **kwargs):
        ...
        if 'group' in newitem.__dict__: newitem.__dict__.pop('group')
        if 'label' in newitem.__dict__: newitem.__dict__.pop('label')
        if 'label_extra' in newitem.__dict__: newitem.__dict__.pop('label_extra')
        if 'context_action' in newitem.__dict__: newitem.__dict__.pop('context_action')
        if 'context_channel' in newitem.__dict__: newitem.__dict__.pop('context_channel')
        for k, v in kwargs.items():
            setattr(newitem, k, v)
        return newitem

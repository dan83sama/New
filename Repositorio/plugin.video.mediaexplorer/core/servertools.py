...
def get_servers_from_id(itemlist, fnc=None, sort=True):
    ...
    :param fnc: funcion encargada de devolver el label para cada item una vez asignado el servidor
    ...
    for item in itemlist:
        ...
        # Asignamos el label
        # TODO: Con el sistema de perfiles en principio esto no se usa
        if fnc:
            item.label = fnc(item)
...
def normalize_url(item):
    ...
    server = moduletools.get_module_parameters(os.path.join(sysinfo.runtime_path, 'servers', item.server + '.json'))
    for pattern in server.get("find_videos", {}).get("patterns", []):
...
def resolve_video_urls(item):
    ...
    server = __import__('servers.%s' % item.server, None, None, ["servers.%s" % item.server])
    ...
    try:
        ...
        result = 'Se ha producido un error en el conector'
        logger.error()
    ...
    if not type(itemlist) == list or not itemlist or not settings.get_setting('resolve_stop', __file__):
        for debrider in debriders:
            ...
            mod = __import__('debriders.%s' % debrider['id'], None, None, ["debriders.%s" % debrider['id']])
    ...
    if not itemlist:
        itemlist = 'No se ha encontrado ningúna url válida'
...
def filter_servers(servers_list):
    """
    Filtra los servidores quitando los que estén deshabilitados en la configuración
    :param servers_list: itemlist con servidores
    :return: itemlist filtrado
    """

    servers_list = filter(
        lambda x: not settings.get_setting(
            "disabled",

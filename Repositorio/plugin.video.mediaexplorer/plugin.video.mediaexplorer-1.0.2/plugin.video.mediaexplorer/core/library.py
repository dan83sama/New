...
def findvideos(item):
    ...
    for source in sources:
        ...
        if i.channel == 'downloads':
            ...
            channel = __import__('channels.%s' % i.channel, None, None, 'channels.%s' % i.channel)
...
def add_to_library(item, silent=False):
    ...
    if item.type == 'movie':
        if not silent:
            ...
            dialog.update(0, 'Obteneiendo información de la película...')
    ...
    if item.type == 'tvshow':
        ...
        filetools.file_open(path_nfo, 'wb').write('https://www.themoviedb.org/tvshow/%s' % data.tmdb_id)
        filetools.file_open(path_json, 'wb').write(data.tojson())
        ...
        if not silent:
            dialog.update(0, 'Obteneiendo temporadas...')

        # Obtenemos las temporadas
        channelmodule = __import__('channels.%s' % item.channel, None, None, 'channels.%s' % item.channel)
        ...
        if datalist:
            for ready, total in mediainfo.get_itemlist_info(datalist):
                if not silent:
                    dialog.update(ready * 100 / total, 'Obteniendo informacion de temporadas...',
                                  '%s de %s' % (ready, total))
        ...
        if not silent:
            dialog.update(0, 'Obteneiendo episodios...')
        ...
        for season in season_list:
            ...
            if not silent:
                dialog.update((season_list.index(season) + 1) * 100 / len(season_list), 'Obteneiendo episodios...')
        ...
        if datalist:
            for ready, total in mediainfo.get_itemlist_info(datalist):
                if not silent:
                    dialog.update(
                        ready * 100 / total,
                        'Obteniendo informacion de episodios...',
                        '%s de %s' % (ready, total)
                    )
        ...
        for x in range(len(episode_list)):
            ...
            if not filetools.isfile(path_json):
                ...
                filetools.file_open(path_nfo, 'wb').write('https://www.themoviedb.org/tvshow/%s' % episode_data.tmdb_id)
...
def delete(item):
    ...
    if platformtools.dialog_yesno(
            'Biblioteca',
            'Estas seguro que quieres eliminar de la bilioteca',
            '\'%s\'' % item.title
    ):

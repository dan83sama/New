...
def get_itemlist_info(itemlist, extended=False, ask=False):
    ...
    s_items = s_items[:int(max_size_list_info)] if max_size_list_info != 'Sin límites' else s_items
...
class TMDb(MediaInfo):
    ...
    def limit_ressults(self, response):
        ...
        # Eliminamos los resultados que no coincidan con el año
        if self.item.year:
    ...
    def _request_api(self, url, post=None):
        ...
        with lock:
            if cache_id not in TEMP_DATA_CACHE:
                TEMP_DATA_CACHE[cache_id] = {'data': None, 'time': time.time()}
            else:
                cached = True

        if cached:
            try:
                while TEMP_DATA_CACHE[cache_id]['date'] is None:
                    time.sleep(0.1)
            except:
                cached = False
            else:
        ...
        if not cached:
            ...
            if req.sucess:
                ...
                TEMP_DATA_CACHE[cache_id]['time'] = time.time()

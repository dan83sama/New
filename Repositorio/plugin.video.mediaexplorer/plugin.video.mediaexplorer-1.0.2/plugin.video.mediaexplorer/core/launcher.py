...
def run(item):
            ...
            if settings.get_setting('debug') == 4:
                ...
                if platformtools.dialog_yesno('MediaExplorer', '¿Quieres compartir el reporte en Pastebin.com?'):
                    ...
                    if not response or "Bad API" in response:
                        ...
                        msg = 'Puedes compartir el reporte desde:\n%s' % response
                        platformtools.dialog_ok('MediaExplorer', msg)
    ...
    # Resto de funciones cargan un listado.
        ...
        # Listado de servidores
            ...
            if not item.channel == 'olimpo' and not item.adult:
                servers = [i.type == 'server' for i in itemlist]
                if True in servers:
                    itemlist.insert(
                        servers.index(True),
                        item.clone(
                            label='Buscar mas servidores en olimpo.link',
                            channel='olimpo',
                            type='highlight'
                        ))
                else:
                    itemlist.append(
                        item.clone(
                            label='Buscar servidores en olimpo.link',
                            channel='olimpo',
                            type='highlight'
                        ))
            ...
            if not item.nofilter:
                ...
                if settings.get_setting('lang_filter_server'):
                    ...
                    filtered_itemlist = filter(
                        lambda x: x.type != 'server' or
                        x.lang.name not in disabled_langs,
                        filtered_itemlist
                    )
                ...
                if settings.get_setting('quality_filter_server'):
                    ...
                    filtered_itemlist = filter(
                        lambda x: x.type != 'server' or
                        x.quality.name not in disabled_qualities,
                        filtered_itemlist
                    )

...
def show_recaptcha(key, referer):
    ...
    :return:
    """
    # TODO: No funciona ya que ahora hay que usar la v2 de la api de google
    from recaptcha import Recaptcha
    skin = 'Estuary' if xbmc.getSkinDir() == 'skin.estuary' else 'Default'
    return Recaptcha("Recaptcha.xml", sysinfo.runtime_path, skin).start(key, referer)
...
def render_items(itemlist, parent_item):
    ...
    if settings.get_setting('media_info', viewtools.__file__) and parent_item.type in ('movie', 'tvshow', 'season', 'episode'):
    ...
    def mark_as_watched_subThread(item):
        ...
        if xbmc.Player().isPlaying():
            try:
                ...
                time_for_watched = eval(time_for_watched[settings.get_setting("time_for_watched", platformsettings.__file__)])

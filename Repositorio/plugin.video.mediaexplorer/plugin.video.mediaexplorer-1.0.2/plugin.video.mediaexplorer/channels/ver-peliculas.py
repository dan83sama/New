...
LNG = Languages({
    ...
    Languages.vos: ['subtituladas', 'subtitulos', 'sub'],
})

QLT = Qualities({
    Qualities.hd: ['HD', '720'],
    Qualities.sd: ['BR'],
    Qualities.scr: ['BR-SCR', 'TS', 'HDTS-SCR', 'DVD-SCR', 'DVD-SCRE', 'HDTS', 'CAM'],
...
def findvideos(item):
    ...
    patron = 'http:\/\/ver-peliculas\.(io|org)\/(?:peliculas|vipdevip)\/(\d+)-(.*?)-\d{4}-online\.'
    url = scrapertools.find_single_match(item.url, patron)

    url = 'http://ver-peliculas.%s/core/api.php?id=%s&slug=%s' % (url[0], url[1], url[2])
...
def play(item):
    logger.trace()
    data = httptools.downloadpage('http://ver-peliculas.io/core/videofinal.php', post=item.post).data

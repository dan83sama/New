...
def play(item):
    ...
    data = scrapertools.find_single_match(data, 'var player_mp4_seek.*?(var.*?);flashvars')
    data = re.sub('/\*.*?\*/', '', data)

    js_vars = {}
    for v in data.split(';'):
        name, value = scrapertools.find_single_match(v,'var ([^=]+)=(.*)')
        values = value.split('+')
        value = ''
        for v in values:
            if not v.strip().startswith('"'):
                value += js_vars[v.strip()]
            else:
                value += v.strip('" ')
        if name.startswith('player_quality_'):
            itemlist.append(Video(url=value, res=name.split('_')[-1]))
        else:
            js_vars[name] = value
